...
bool pent_term(int *f_index, const Fullerene(&F), Clar_struct(&S),
...
//    return;
//
//  // lets check if *f_index has already been assigned (and is therefore non-res)
//  if (cur_sol.assigned_f[*f_index] != 0) {
//#if DEBUGCLAR
...
//    // move onto next pentagon
//    assign_res_pent(f_index + 1, f_count + 1, F ,cur_sol, best_sol, hex_ids, edge_ids, 
//                    p, pents_done);
//    // make face non-resonant
//    change_res_of_face(*f_index, F.dual[*f_index], cur_sol, F.primal, false);
//
